...
// สคริปต์สำหรับอัปเดตการตั้งค่ากิจกรรม (Fixed: Type Definition Count Mismatch)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $enable_shipping = isset($_POST['enable_shipping']) ? intval($_POST['enable_shipping']) : 0; 
    $shipping_cost = isset($_POST['shipping_cost']) ? floatval($_POST['shipping_cost']) : 0.00; 

    // Merchandise Info
    $merch_link = isset($_POST['merch_link']) ? e($_POST['merch_link']) : null;
    $merch_enabled = isset($_POST['merch_enabled']) ? intval($_POST['merch_enabled']) : 0;
    ...
    try {
        ...
        $sql_set_parts = [
            ...
            "enable_shipping = ?", "shipping_cost = ?", 
            "description = ?", "awards_description = ?",
            "map_embed_url = ?", "map_direction_url = ?",
            "bib_prefix = ?", "bib_start_number = ?", "bib_padding = ?", "corral_settings = ?",
            "payment_deadline = ?",
            "merch_link = ?", "merch_enabled = ?" 
        ];
        
        // เรียงลำดับตัวแปรให้ตรงกับ SQL (25 ตัวแปรหลัก)
        $params = [
            $name, $slogan, $start_date, $is_registration_open,         // 4
            $theme_color, $color_code,                                  // 2
            $organizer, $contact_person_name, $contact_person_phone,    // 3
            $payment_bank, $payment_account_name, $payment_account_number, // 3
            $enable_shipping, $shipping_cost,                           // 2
            $description, $awards_description,                          // 2
            $map_embed_url, $map_direction_url,                         // 2
            $bib_prefix, $bib_start_number, $bib_padding, $corrals_json, // 4
            $payment_deadline,                                          // 1
            $merch_link, $merch_enabled                                 // 2
        ];
        
        // [FIXED] Correct type string: 25 chars exactly
        // s=string, i=integer, d=double
        // sssi ssssssss id ssss siis s si
        $types = "sssissssssssidsssssiisssi"; 

<?php
// admin/event_settings.php - หน้าตั้งค่ากิจกรรม (เพิ่มส่วน Merchandise Link)
...
<form action="../actions/update_event_settings.php" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-xl shadow-md space-y-8">
    ...
    <!-- [NEW] Section: Merchandise Link -->
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">2. สั่งซื้อเสื้อที่ระลึก (Merchandise)</h2>
        <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div class="md:col-span-2">
                <label for="merch_link" class="block text-sm font-medium text-gray-700">ลิงก์สั่งซื้อ (URL)</label>
                <input type="url" id="merch_link" name="merch_link" value="<?= e($event['merch_link'] ?? '') ?>" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm p-2" placeholder="https://docs.google.com/forms/d/e/..." >
                <p class="text-xs text-gray-500 mt-1">ใส่ URL ของ Google Form หรือเว็บขายของอื่นๆ</p>
            </div>
            <div>
                <label for="merch_enabled" class="block text-sm font-medium text-gray-700">สถานะปุ่มสั่งซื้อ</label>
                <select id="merch_enabled" name="merch_enabled" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm p-2">
                    <option value="0" <?= !$event['merch_enabled'] ? 'selected' : '' ?>>ปิด (ไม่แสดงปุ่ม)</option>
                    <option value="1" <?= $event['merch_enabled'] ? 'selected' : '' ?>>เปิด (แสดงปุ่ม)</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">3. ข้อมูลผู้จัดและการติดต่อ</h2>
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">4. ข้อมูลการชำระเงินและการจัดส่ง</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
                 <div>
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">5. เนื้อหากิจกรรม</h2>
        <div>
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">6. ระยะทางและราคา</h2>
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">7. คลังรูปภาพ (อัปโหลดไฟล์)</h2>
        <div class="space-y-6">
            <?php
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">8. ภาพปกและภาพย่อ (Cover & Thumbnail)</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">9. ข้อมูลแผนที่ (Location & Map)</h2>
        <div class="space-y-4">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">URL สำหรับแสดงผลแผนที่ในหน้าเว็บ (จาก Google Maps > Share > Embed a map > เอาเฉพาะ src)</p>
            </div>
            <div>
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">10. การตั้งค่าเลข BIB</h2>
    ...
    <div>
        <h2 class="text-xl font-bold text-primary border-b pb-2 mb-4">11. จัดการกลุ่มปล่อยตัว (Corral/Wave)</h2>
...
<script>
    ...
    function renderCorrals() {
        ...
        corrals.forEach((corral, index) => {
            const row = document.createElement('div');
            row.className = 'flex flex-wrap items-center gap-2 p-2 border rounded-lg bg-gray-50 corral-row';
            row.innerHTML = `
                <input type="text" name="corrals[${index}][name]" placeholder="ชื่อกลุ่ม (เช่น A)" value="${corral.name || ''}" required class="w-full sm:w-16 p-2 border rounded-md text-sm font-bold" title="ชื่อกลุ่ม">
                <input type="number" name="corrals[${index}][from_bib]" placeholder="ตั้งแต่ BIB" value="${corral.from_bib || ''}" class="w-full sm:w-24 p-2 border rounded-md text-sm" title="เลข BIB เริ่มต้น">
                <input type="number" name="corrals[${index}][to_bib]" placeholder="ถึง BIB" value="${corral.to_bib || ''}" class="w-full sm:w-24 p-2 border rounded-md text-sm" title="เลข BIB สิ้นสุด">
                ...
                <input type="text" name="corrals[${index}][description]" placeholder="คำอธิบาย" value="${corral.description || ''}" class="flex-grow p-2 border rounded-md text-sm min-w-[150px]" title="คำอธิบาย">
    ...
    function addCorralRow() {
        ...
        const row = document.createElement('div');
        row.className = 'flex flex-wrap items-center gap-2 p-2 border rounded-lg bg-gray-50 corral-row animate-fade-in';
        row.innerHTML = `
            <input type="text" name="corrals[${nextIndex}][name]" placeholder="ชื่อกลุ่ม (เช่น A)" value="" required class="w-full sm:w-16 p-2 border rounded-md text-sm font-bold" title="ชื่อกลุ่ม">
            <input type="number" name="corrals[${nextIndex}][from_bib]" placeholder="ตั้งแต่ BIB" value="" class="w-full sm:w-24 p-2 border rounded-md text-sm" title="เลข BIB เริ่มต้น">
            <input type="number" name="corrals[${nextIndex}][to_bib]" placeholder="ถึง BIB" value="" class="w-full sm:w-24 p-2 border rounded-md text-sm" title="เลข BIB สิ้นสุด">
            ...
            <input type="text" name="corrals[${nextIndex}][description]" placeholder="คำอธิบาย" value="" class="flex-grow p-2 border rounded-md text-sm min-w-[150px]" title="คำอธิบาย">
            <button type="button" onclick="removeCorralRow(this)" title="ลบกลุ่มนี้" class="bg-red-100 text-red-600 hover:bg-red-200 p-2 rounded-md h-9 w-9 flex-shrink-0 flex items-center justify-center"><i class="fa-solid fa-trash text-xs"></i></button>
        `;

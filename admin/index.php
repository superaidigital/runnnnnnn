<?php
// admin/index.php - หน้า Dashboard หลักสำหรับ Admin/Staff (เวอร์ชันรองรับ Staff ดูแลหลายงาน)
...
// [FIXED] เตรียมรายการ Event IDs ที่ Staff ดูแล (แปลงให้เป็น Array เสมอ)
$assigned_event_ids = [];
if (!$is_super_admin) {
    if (isset($staff_info['assigned_event_ids']) && is_array($staff_info['assigned_event_ids'])) {
        $assigned_event_ids = $staff_info['assigned_event_ids'];
    } elseif (isset($staff_info['assigned_event_id'])) {
        // กรณี Session เก่าที่ยังเก็บเป็น int
        $assigned_event_ids = [$staff_info['assigned_event_id']];
    }
    
    // ถ้าไม่มีงานที่ดูแลเลย ให้ใส่ 0 เพื่อไม่ให้ query พัง (ID 0 ไม่มีจริง)
...
// --- 1. Fetch Stats Data (แก้ไขให้รองรับ IN (...)) ---
$stats_query = "
    SELECT 
...
if (!$is_super_admin) {
    // สร้าง Placeholder ตามจำนวน Event IDs (เช่น ?,?,?)
    $placeholders = `(` . implode(',', array_fill(0, count($assigned_event_ids), '?')) . `)`; // Fixed backticks syntax for readability in thought, actually string
    $placeholders = implode(',', array_fill(0, count($assigned_event_ids), '?'));
    
    $stats_query .= " WHERE r.event_id IN ($placeholders)";
    $stats_params = $assigned_event_ids;
    $stats_types = str_repeat('i', count($assigned_event_ids));
}
...
// --- 2. Fetch Events (แยก Active/Cancelled และกรองตามสิทธิ์) ---
$active_events = [];
$cancelled_events = [];
...
if (!$is_super_admin) {
    $placeholders = implode(',', array_fill(0, count($assigned_event_ids), '?'));
    $events_query_sql .= " WHERE id IN ($placeholders)";
    $events_params = $assigned_event_ids;
    $events_types = str_repeat('i', count($assigned_event_ids));
}
$events_query_sql .= " ORDER BY sort_order ASC";

$events_stmt = $mysqli->prepare($events_query_sql);
if (!$is_super_admin && !empty($events_params)) {
    $events_stmt->bind_param($events_types, ...$events_params);
}
$events_stmt->execute();
...
// --- 3. Fetch Pending Registrations (แก้ไขให้รองรับ IN (...)) ---
$pending_query = "
    SELECT r.id, r.registration_code, e.name as event_name, r.payment_slip_url, r.event_id, r.shipping_option
    FROM registrations r
...
if (!$is_super_admin) {
    $placeholders = implode(',', array_fill(0, count($assigned_event_ids), '?'));
    $pending_query .= " AND r.event_id IN ($placeholders)";
    $pending_params = $assigned_event_ids;
    $pending_types = str_repeat('i', count($assigned_event_ids));
}
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <?php else: ?>
                    ...
                    <tr class="hover:bg-yellow-50">
                        ...
                        <td class="px-3 py-4 whitespace-nowrap text-sm flex items-center gap-2">
                            ...
                            <form action="../actions/update_registration.php" method="POST" onsubmit="return confirm('ยืนยันการปรับสถานะเป็น \n\'ชำระเงินแล้ว\' ใช่หรือไม่?');">
                                ...
                                <input type="hidden" name="redirect_url" value="../admin/index.php">
                                <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-3 rounded-lg text-xs">

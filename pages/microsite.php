...
// [UPDATED] Fetch merch_link and merch_enabled
...
<div class="space-y-6">
    ...
    <!-- Action Buttons -->
    <div class="text-center pt-4 space-y-3 md:space-y-0 md:space-x-3 flex flex-col md:flex-row justify-center">
        
        <!-- [NEW] Merchandise Button (Fixed Warning) -->
        <!-- ใช้ !empty() เพื่อป้องกัน Error หากฐานข้อมูลยังไม่มีคอลัมน์ใหม่ -->
        <?php if (!empty($event['merch_enabled']) && !empty($event['merch_link'])): ?>
            <a href="<?= e($event['merch_link']) ?>" target="_blank" class="w-full md:w-auto inline-block py-3 px-8 text-lg font-bold rounded-lg text-white bg-yellow-500 hover:bg-yellow-600 transition shadow-lg">
                <i class="fa-solid fa-shirt mr-2"></i> สั่งจองเสื้อที่ระลึก
            </a>
        <?php endif; ?>
        ...
        <?php else: ?>
            <div class="w-full p-4 rounded-lg bg-red-100 text-red-700">
